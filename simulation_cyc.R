...
t0 <- proc.time()[3]
for (r in 1:replicates){
  res_int[[r]] <- ricf_R_int(L=t(models[[r]]$B), data=t(models[[r]]$Y), targets=models[[r]]$targets, target.length=models[[r]]$target.length)
}
t2 <- (proc.time()[3] - t0) #/ replicates

t0 <- proc.time()[3]
for (r in 1:replicates){
  res_int2[[r]] <- ricf_int(L=t(models[[r]]$B), data=t(models[[r]]$Y), targets=models[[r]]$targets, target.length=models[[r]]$target.length)
}
t3 <- (proc.time()[3] - t0) #/ replicates



sum(sapply(1:replicates, function (r) res_int[[r]]$converged == T))
sum(sapply(1:replicates, function (r) res_int2[[r]]$converged == T))
sum(sapply(1:replicates, function (r) res_dg_agg[[r]]$allconverged == T))
sum(sapply(1:replicates, function (r) res_int[[r]]$converged & res_int2[[r]]$converged & res_dg_agg[[r]]$allconverged == T))
...
rss_sigma <- function(r, List){
  sigma <- solve(diag(v) - models[[r]]$B.true) %*% models[[r]]$Omega.true %*% t(solve((diag(v)-models[[r]]$B.true)))
  rss <- sum((sigma-List[[r]]$Sigmahat)^2) + sum((diag(sigma-List[[r]]$Sigmahat))^2)
  num <- 2 * choose(nrow(models[[r]]$B.true)+1,2)
  return (rss / num)
}
...
mean(sapply(1:replicates, rss, List=res_dg_agg))
mean(sapply(1:replicates, rss, List=res_int))
mean(sapply(1:replicates, rss, List=res_int2))

mean(sapply(1:replicates, d_llh, List=res_dg_agg))
...
#mean(sapply(1:replicates, rss_sigma, List=res_dg_agg))
#mean(sapply(1:replicates, rss_sigma, List=res_int))
#mean(sapply(1:replicates, rss_sigma, List=res_int2))
#mean(sort(sapply(1:replicates, rss_sigma, List=res_dg_agg))[1:(replicates-5)])
#mean(sort(sapply(1:replicates, rss_sigma, List=res_int))[1:(replicates-5)])

#max(sapply(1:replicates, rss_sigma, List=res_dg_agg))
#max(sapply(1:replicates, rss_sigma, List=res_int))
#hist(sapply(1:replicates, rss_sigma, List=res_dg_agg), breaks=100)
#hist(sapply(1:replicates, rss_sigma, List=res_int), breaks=100)

#for (i in 1:replicates) {if (abs(rss(i, res_int)-rss(i, res_int2))>1e-6) break}
path = paste0("results/p=",v, "n=", n, "l=", l, "d=", d, ".RData")
#save(res_dg_agg, res_int, res_int2, t1, t2, t3, file=path)
t1
t2
t3

max(sapply(1:replicates, rss, List=res_int2))
which.max(sapply(1:replicates, rss, List=res_int2))
#max(sapply(1:replicates, rss_sigma, List=res_int2))
#which.max(sapply(1:replicates, rss_sigma, List=res_int2))
max(sapply(1:replicates, d_llh, List=res_int2))
which.max(sapply(1:replicates, d_llh, List=res_int2))
#min(sapply(1:replicates, function(r) det(diag(v)-models[[r]]$B.true)))
